...
describe('Password Generator', () => {
    ...
    it('includes uppercase letters when requested', () => {
        const password = generatePassword({ length: 100, uppercase: true, lowercase: true, numbers: false, symbols: false });
        expect(password).toMatch(/[A-Z]/);
    });

    it('includes numbers when requested', () => {
        const password = generatePassword({ length: 100, uppercase: false, lowercase: true, numbers: true, symbols: false });
        expect(password).toMatch(/[0-9]/);
    });

    it('includes symbols when requested', () => {
        const password = generatePassword({ length: 100, uppercase: false, lowercase: true, numbers: false, symbols: true });
